...
desc "Generate a new libffi shell script"
task :new, [:version, :stack] do |t, args|
  write_file = Proc.new do |version, stack|
    file = "libffi/#{args[:stack]}/libffi-#{args[:version]}.sh"
    puts "Writing #{file}"
...
docker run -v `pwd`/builds:/tmp/output -e VERSION=#{args[:version]} -e STACK=#{args[:stack]} hone/libffi-builder:#{args[:stack]}
FILE
    end
...
task :upload, [:version, :stack, :staging] do |t, args|
  ...
  filename     = "libffi-#{args[:version]}.tgz"
...
task :generate_image, [:stack] do |t, args|
  ...
  system("docker build -t hone/libffi-builder:#{args[:stack]} .")
